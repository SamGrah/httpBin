...
{#if !$binRequestHistory?.length}
	<div class="max-w-md py-4 px-8 bg-white shadow-lg rounded-lg my-20">
		<div>
			<h2 class="text-gray-800 text-3xl font-semibold">Bin {data.binId} is Empty</h2>
			<p class="mt-4 text-gray-600">
				No HTTP requests have been recieved by bin {data.binId}.<br />
				A request of any type (<i>GET</i>, <i>DELETE</i>, etc) can be added to this bin by making a
				request to the following address.
			</p>
			<br />
			<div class="flex justify-center mt-4.mb-3">
				<a class="text-xl font-medium text-blue-900" href=".">{data.hostname}/bin/{data.binId}</a>
			</div>
		</div>
	</div>
{:else}
  <ul>
    {#each $binRequestHistory as request}
      {#each Object.entries(request.contents) as key, value}
